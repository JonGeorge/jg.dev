...
The thing is, those fights were productive. Every analysis function ended up returning borrowed data tied to the `&Matrix{:rust}` it was handed, `Vec<(&Stressor, u32)>{:rust}`, clusters of `&Component{:rust}`, etc. and the all-pairs coupling computation was where ownership genuinely mattered. When the borrow checker complained, it was really asking "*who owns this data, and how long does it live?*" That's an architecture question. Answering it up front is why the analysis module ended up pure. Functions over `&Matrix{:rust}`, no IO, fully unit-tested. `storage.rs` does the file work. And `src/commands` just wires things together. The language nudged me toward the separation I'd wanted anyway, and then wouldn't let me abandon it.
